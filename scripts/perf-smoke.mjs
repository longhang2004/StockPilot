 ...
 *                       limiter (60/min per client), so the write workload is
...
// The rate-limit check crosses the auth-tier threshold (60/min) by design;
...
// Auth-tier limiter ceiling for /v1/auth/demo-login: 60/min per client.
...
async function main() {
  ...
  if (jsonOutput) {
    ...
    if (includeWrites) {
      console.log(
        `note: demo-login capped at ${WRITE_WORKLOAD_REQUEST_CAP} requests to stay under the 60/min auth-tier limiter`,
      );
    }
    console.log(
      [
