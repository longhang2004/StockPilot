...
# Generated files that are git-ignored and recreated by the toolchain; they
# must never be handed off in an archive. Keyed by path relative to the
# repository root.
EXCLUDED_RELATIVE_FILE_PATHS = {
    "apps/web/next-env.d.ts",  # regenerated by `next typegen`
}
...
def collect_files(repo_root, output_path):
    ...
    for root, dirs, files in os.walk(repo_root, topdown=True):
        ...
        for f in files:
            ...
            if rel_file_path.as_posix() in EXCLUDED_RELATIVE_FILE_PATHS:
                continue
...
def compress_repo(repo_root, output_path, fmt="zip"):
    """Archive `repo_root` to `output_path`, excluding build artifacts,
    environment files, generated files, and archives.

    `repo_root` is an explicit parameter so callers (including the self-test)
    can archive any directory — never a hidden module-level constant.
    """
    repo_root = Path(repo_root).resolve()
    output_path = Path(output_path).resolve()
...
def self_test():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixtures = {
            ...
            "notes.txt": "hello\n",
            # Excluded build/output directories must not be traversed.
            "node_modules/pkg/index.js": "module.exports = 1;\n",
            ".next/static/chunk.js": "// build output\n",
            "dist/server.js": "// build output\n",
            "coverage/lcov.info": "SF:src/app.ts\n",
            "playwright-report/index.html": "<html></html>\n",
            "test-results/e2e.log": "server log\n",
            # Generated, git-ignored Next.js file that must never be archived.
        ...
        check(
            "backup.zip" not in names,
            "the output archive itself must never be included",
        )
        excluded_dirs = {
        ...
        check(
            not (names & excluded_dirs),
            f"excluded build directories leaked into the archive: {sorted(names & excluded_dirs)}",
        )
        check(
            "apps/web/next-env.d.ts" not in names,
            "generated apps/web/next-env.d.ts must be excluded from archives",
        )

        # Round-trip through the real write path (zip) and confirm the same
        # set. The fixture root is passed explicitly so the self-test archives
        # the temporary tree — never the real repository.
        compress_repo(root, output, fmt="zip")
        ...
        # The written archive must match the collected set exactly (no
        # fixture file silently dropped or added by the write path).
        check(
            archived == names,
            f"written archive differs from collected set: only-archived={sorted(archived - names)} only-collected={sorted(names - archived)}",
        )
...
def main():
    ...
    compress_repo(REPO_ROOT, args.output, fmt=args.format)
